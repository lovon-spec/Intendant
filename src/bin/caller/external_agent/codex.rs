...
- For browser/dashboard validation against an existing Intendant web port, prefer `node scripts/validate-dashboard.cjs --port <port> --selector <css>` or `--url <url>` before writing ad-hoc Chromium/CDP scripts. For Station smoke checks, prefer named probes such as `--station-probe rendered --station-probe dock-hidden` over giant inline `--wait-for-function` expressions. For headed Station QA, use the helper's first-class artifacts and rendered controls probe against realistic state: require non-empty or explicitly seeded Activity/events plus active managed/context state, a real provider/model/session, and the expected external Codex backend, for example `--headed --require-station-state --require-managed-context-state --require-ai-provider-session --require-external-agent codex --station-interaction-probe --screenshot /tmp/intendant-station.png --json`; read the returned artifact path, interaction names/count, warm-up latency, subsequent latency, and review the screenshot path before counting the QA pass. For a temporary local dashboard smoke, prefer one owned-lifecycle command such as `node scripts/validate-dashboard.cjs --launch-dashboard --port <throwaway_port> --selector <css>`; the helper launches the built intendant binary as `--web <port> --no-tui`, waits for HTTP readiness, and stops it afterward. For separate headed CU/browser E2E against a temporary dashboard, run `node scripts/validate-dashboard.cjs --hold-dashboard --port <throwaway_port> --json` as a foreground long-running command, use the printed URL for CU/browser steps while that command stays active, then interrupt the command for helper-owned cleanup. Do not start a separate foreground/nohup/setsid dashboard just so another tool can connect. The helper launches isolated Chromium for validation mode, waits for CDP and selectors/functions/probes, handles WebSocket fallbacks, captures requested artifacts, and prints compact PASS/FAIL output with bounded failure excerpts.
