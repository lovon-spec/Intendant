...
Intendant-supervised Codex `app-server` launches suppress inherited user-global
Codex MCP/plugin/app servers by default and inject Intendant's MCP endpoint plus
the explicit toggles above. Set `INTENDANT_CODEX_INHERIT_MCP_SERVERS=1` only for
a launch that should inherit the user's configured Codex MCP servers and plugins.
